...
def checkout(request):
    ...
    if request.method == 'POST':
        try:
            # Transaction.atomic asegura que si algo falla, no se guarde ningún cambio corrupto en la BD
            with transaction.atomic():
                ...
                for product_id in valid_ids:
                    ...
                    # select_for_update() bloquea el registro en PostgreSQL hasta que la compra termine (Previene fallos de inventario simultáneos)
                    product = Product.objects.select_for_update().get(id=product_id)
                    
                    if product.stock < quantity:
                        raise ValueError(f'No hay suficiente stock para "{product.name}". Solo quedan {product.stock} unidades.')
                ...
                request.session['cart'] = {} # Vaciamos el carrito tras pagar
                messages.success(request, f'¡Compra realizada con éxito! Tu número de orden es #{order.id}.')
                return redirect('product_list')
    ...
    return render(request, 'products/checkout.html', {'cart_count': sum(cart.values())})
